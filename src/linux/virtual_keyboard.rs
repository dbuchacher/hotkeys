...
use std::{sync::Mutex, error::Error, process};
use evdev::{uinput::*, Key, AttributeSet, RelativeAxisType};

// Virtual Device; keyboard and mouse events will be sent from this
pub static VD: Lazy<Mutex<VirtualDevice>> = Lazy::new(|| Mutex::new(
    virtual_keyboard().unwrap_or_else(|err| {
        eprintln!("Failed Creating Virtual Keyboard: {}", err);
        process::exit(1);
    })
));

pub fn virtual_keyboard() -> Result<VirtualDevice, Box<dyn Error>> {
    // holds mouse data like its x/y postion
    let mut axes: AttributeSet<RelativeAxisType> = AttributeSet::<RelativeAxisType>::new();

    // holds keys that can be sent like `enter` or `left click`
    let mut keys: AttributeSet<Key> = AttributeSet::<Key>::new();
    ...
    axes.insert(RelativeAxisType::REL_MISC);
    axes.insert(RelativeAxisType::REL_RESERVED);
    ...
    keys.insert(Key::KEY_ROTATE_DISPLAY);
    ...
    keys.insert(Key::KEY_FULL_SCREEN);
    ...
    keys.insert(Key::KEY_NUMERIC_A);
    keys.insert(Key::KEY_NUMERIC_B);
    keys.insert(Key::KEY_NUMERIC_C);
    keys.insert(Key::KEY_NUMERIC_D);
    ...
    keys.insert(Key::KEY_ASSISTANT);
    keys.insert(Key::KEY_KBD_LAYOUT_NEXT);
    ...
    keys.insert(Key::KEY_RIGHT_UP);
    keys.insert(Key::KEY_RIGHT_DOWN);
    keys.insert(Key::KEY_LEFT_UP);
    keys.insert(Key::KEY_LEFT_DOWN);
    keys.insert(Key::KEY_ROOT_MENU);
    keys.insert(Key::KEY_MEDIA_TOP_MENU);
    keys.insert(Key::KEY_NUMERIC_11);
    keys.insert(Key::KEY_NUMERIC_12);
    keys.insert(Key::KEY_AUDIO_DESC);
    keys.insert(Key::KEY_3D_MODE);
    keys.insert(Key::KEY_NEXT_FAVORITE);
    keys.insert(Key::KEY_STOP_RECORD);
    keys.insert(Key::KEY_PAUSE_RECORD);
    keys.insert(Key::KEY_VOD);
    keys.insert(Key::KEY_UNMUTE);
    keys.insert(Key::KEY_FASTREVERSE);
    keys.insert(Key::KEY_SLOWREVERSE);
    keys.insert(Key::KEY_DATA);
    keys.insert(Key::KEY_ONSCREEN_KEYBOARD);
    keys.insert(Key::KEY_PRIVACY_SCREEN_TOGGLE);
    keys.insert(Key::KEY_SELECTIVE_SCREENSHOT);
