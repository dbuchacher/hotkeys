...
// invoked in the `main` function after setting KEYBINDS to start event monitoring loop
    ...
    loop {
        for event in device.fetch_events().unwrap() {
            match event.event_type().0 {
                // 1: if no KEYBINDS are triggered we will send the event as normal. (or if unblock is set to true)
                1 => if check_keybinds(event) {
...
fn check_keybinds(event: InputEvent) -> bool {
    // // failsafe exit
    // if event.code() == 19 {
    //     if  {
    //         println!("fuck");
    ...
    let keys = KEYBINDS.lock().unwrap().clone();

    // loop through all user set KEYBINDS
    for key in keys {
        ...
        if logic {
            match key.action {
                Action::None => (),
                Action::Func => key.func.unwrap()(),
                Action::Send => VD.lock().unwrap().emit(&key.send.unwrap()).unwrap(),
            }
            if key.unblock { 
